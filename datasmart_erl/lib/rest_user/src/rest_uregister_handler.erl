...
-record(state, {
  method, isAuthorized = true, etag,
  email, password, registred
}).
...
allowed_methods(Req, State) ->
  ...
  {[<<"POST">>, <<"PUT">>, <<"OPTIONS">>], Req, NewState}.

%% FLOW: ALL[5] OR 400
%% Return whether the request is malformed.
malformed_request(Req, State) ->
  #{email := EmailBin, password := PasswordBin} = cowboy_req:match_qs([
    {email, [], undefined},
    {password, [], undefined}
  ], Req),
  ...
  case EmailBin =/= undefined andalso
    ...
    true ->
      ...
      NewState = State#state{email = Email, password = Password},
      {false, Req, NewState};
...
handle(Req, State#state{method = <<"POST">>, email = Email, password = Password}) ->
  case user_server:register(Email, Password) of
    {ok, Registred} ->
      NewState = State#state{registred = Registred},
      {jiffy:encode({Registred}), Req, NewState}
  end.
