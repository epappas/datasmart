...
main(_) ->
  ...
  GenerateOUKeyKVList = test_generate(oukey, []),
  GenerateAUKeyKVList = test_generate(aukey, [lists:keyfind(oukey, 1, GenerateOUKeyKVList)]),
  _GenerateATokenKVList = test_generate(atoken, [lists:keyfind(aukey, 1, GenerateAUKeyKVList)]),
...
test_generate(oukey, []) ->
  ...
  UKey = hash_md5:build(binary_to_list(uuid:uuid1())),

  {ok, ResultList} = user_server:generate({oukey, [
    {ukey, UKey},
    {email, Email}
  ]}),
...
test_generate(aukey, [{oukey, OUKey}]) ->

  {ok, ResultList} = user_server:generate({aukey, [
    {oukey, OUKey}
  ]}),
...
test_generate(atoken, [{aukey, AUKey}]) ->

  {ok, ResultList} = user_server:generate({atoken, [
    {aukey, binary_to_list(AUKey)}
  ]}),

  etap:is(lists:keymember(atoken, 1, ResultList), true, "Should have return an atoken"),
  etap:is(lists:keymember(opensalt, 1, ResultList), true, "Should have return an opensalt"),
  etap:is(lists:keymember(scope, 1, ResultList), true, "Should have return an scope"),
  ResultList.
